...
app.post('/', async (c) => {
  ...
  try {
    ...
    const elapsed = ((performance.now() - startTime) / 1000).toFixed(1)
    logTaskError('Chat', 'chat', { elapsedSeconds: elapsed, error: err.message })
...
app.get('/messages', async (c) => {
  ...
  const rows = db.select().from(schema.chatMessages)
    .where(and(eq(schema.chatMessages.dramaId, dramaId), eq(schema.chatMessages.episodeId, episodeId)))
