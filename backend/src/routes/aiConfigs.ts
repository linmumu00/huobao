  ...
  if (p === "gemini") {
    const url = new URL(
      joinProviderUrl(
        baseUrl,
        "/v1beta",
        `/models/${m || "gemini-2.5-flash"}:generateContent`,
      ),
    );
...
app.post("/", async (c) => {
  ...
  const res = db
    .insert(schema.aiServiceConfigs)
    .values({
...
app.post("/test", async (c) => {
  ...
  console.log("lgb", probe);
  console.log("lgb", probe.url);
  console.log("lgb", probe.method);
  console.log("lgb", probe.headers);
  console.log("lgb", probe.body);
  try {
