  ...
  <div class="settings-layout">
      ...
      <div v-if="tab === 'ai'" class="settings-scroll">
        <div class="settings-head">
          <div class="settings-brand">
            ...
            <div class="settings-brand-copy">
              ...
              <div class="settings-brand-name">短剧工作台</div>
            </div>
          </div>
        ...
        <section class="setup-panel card">
          ...
          <div class="preset-grid">
            <article
              v-for="preset in huobaoPresetCards"
              :key="preset.serviceType"
              class="preset-card"
            >
        ...
        <section class="setup-panel card">
          <div class="setup-panel-head compact">
            <div>
              ...
              <div class="setup-desc">
                选择服务类型后，直接用模板填充推荐的 `provider / base URL /
                model`。
              </div>
            </div>
          </div>
        ...
        <div class="sections">
          <section v-for="st in serviceTypes" :key="st.type">
            <div class="section-head">
              <div>
                ...
                <div class="section-subtitle">
                  {{ serviceMeta[st.type].desc }}
                </div>
              </div>
              <span v-if="countActive(st.type)" class="tag tag-accent"
                >{{ countActive(st.type) }} 已启用</span
              >
              <button
            ...
            <div class="config-list">
              <div
                v-for="c in byType(st.type)"
                :key="c.id"
                class="card config-row"
              >
                  ...
                  <div class="config-main">
                    <div class="config-line">
                      ...
                      <span class="config-name">{{
                        c.name || `${c.provider}-${c.service_type}`
                      }}</span>
                    </div>
                    <span class="config-model mono truncate">{{
                      fmtModel(c.model)
                    }}</span>
                    <span class="config-base mono truncate">{{
                      c.base_url || "未设置 Base URL"
                    }}</span>
                  </div>
                </div>
                <span
                  :class="['tag', c.api_key ? 'tag-success' : 'tag-error']"
                  >{{ c.api_key ? "已配置" : "无密钥" }}</span
                >
                <button
                  class="btn btn-ghost btn-sm"
                  @click="testExistingCfg(c)"
                >
                  测试
                </button>
                <label class="toggle"
              ...
              <p v-if="!byType(st.type).length" class="config-empty">
                暂无配置
              </p>
            </div>
          </section>
        ...
        <div class="agent-list">
          <div v-for="a in agentDefs" :key="a.type" class="card agent-card">
            <div class="agent-card-head" @click="toggleAgentEdit(a.type)">
              ...
              <span v-if="getAgentCfg(a.type)" class="tag tag-success"
                >已配置</span
              >
              ...
              <ChevronDown
                :size="14"
                :style="{
                  transform: editingAgent === a.type ? 'rotate(180deg)' : '',
                  transition: '0.2s',
                }"
              />
            </div>
            <div v-if="editingAgent === a.type" class="agent-card-body">
              <label class="field">
                <span class="field-label"
                  >模型 <span class="dim">(留空使用 AI 服务默认)</span></span
                >
                <BaseSelect
              ...
              <div class="agent-card-foot">
                <button
                  class="btn btn-ghost btn-sm"
                  @click="resetAgentPrompt(a.type)"
                >
                  恢复默认
                </button>
                <span
                  v-if="agentSaved === a.type"
      ...
      <div v-else-if="tab === 'skills'" class="skills-layout">
        ...
        <aside class="skills-agent-list">
          ...
          >
            ...
            <span
              v-if="agentSkillCount(a.type) > 0"
              class="skill-count-badge"
              >{{ agentSkillCount(a.type) }}</span
            >
          </button>
        </aside>
          ...
          <div class="settings-head">
            ...
            <div style="display: flex; align-items: center; gap: 10px">
              <span
                ...
                >{{ selectedAgentIcon }}</span
              >
              <div>
                <h2 class="settings-title" style="margin: 0">
                  {{ selectedAgentLabel }}
                </h2>
            ...
            <div v-for="s in currentSkills" :key="s.id" class="card skill-card">
              <div class="skill-card-head" @click="toggleSkillEdit(s.id)">
                ...
                <ChevronDown
                  :size="14"
                  :style="{
                    transform: editingSkill === s.id ? 'rotate(180deg)' : '',
                    transition: '0.2s',
                  }"
                />
              </div>
              <div v-if="editingSkill === s.id" class="skill-card-body">
                ...
                <div class="skill-card-foot">
                  <span class="dim" style="font-size: 11px"
                    >skills/{{ selectedAgentType }}/{{ s.id }}/SKILL.md</span
                  >
                  <span
                    v-if="skillSaved === s.id"
                    class="tag tag-success"
                    style="margin-left: 8px"
                  >
                    <Check :size="10" /> 已保存
    ...
    <div v-if="cfgDialog" class="overlay" @click.self="cfgDialog = false">
      <form class="modal card config-modal" @submit.prevent="saveCfg">
        <div class="config-modal-head">
          <div>
            <div class="setup-kicker">
              {{ cfgEditId ? "Edit Config" : "New Config" }}
            </div>
            <h2 class="modal-title">
              {{
                cfgEditId
                  ? "编辑服务配置"
                  : `添加${serviceMeta[cfgForm.service_type].label}服务`
              }}
            </h2>
          ...
          <span class="tag tag-accent">{{
            serviceMeta[cfgForm.service_type].label
          }}</span>
        </div>
        ...
        <label class="field">
          ...
          <input
            v-model="cfgForm.name"
            class="input"
            placeholder="如 火宝默认图像服务"
          />
        </label>
        <label class="field"
        ...
        <label class="field">
          ...
          <input
            v-model.number="cfgForm.priority"
            class="input"
            type="number"
            min="0"
            max="999"
          ...
          <span class="field-hint"
            >数值越高越优先。工作台默认会优先使用同类型里优先级最高的启用配置。</span
          >
        </label>
        <label class="field"
        ...
        <div
          ...
          class="test-result"
          :class="{
            ok: cfgTestResult.reachable,
            bad: !cfgTestResult.reachable,
          }"
        >
          <div class="test-result-head">
            <span
              class="tag"
              :class="cfgTestResult.reachable ? 'tag-success' : 'tag-error'"
              >{{ cfgTestResult.status || "ERROR" }}</span
            >
            <span>{{ cfgTestResult.message }}</span>
          </div>
          <div class="mono test-result-url">
            {{ cfgTestResult.method }} {{ cfgTestResult.url }}
          </div>
          <div
            v-if="cfgTestResult.response_preview"
            class="mono test-result-preview"
          >
            {{ cfgTestResult.response_preview }}
          </div>
        </div>
        <div class="modal-actions">
          ...
          <button type="button" class="btn" @click="cfgDialog = false">
            取消
          </button>
      ...
      <form class="modal card config-modal" @submit.prevent="applyHuobaoPreset">
        <div class="config-modal-head">
          <div>
            ...
            <div class="modal-note">
              按火宝推荐链路自动创建或更新 4 条配置：文本 / 图片 / 视频 / 音频。
            </div>
          </div>
        ...
        <div class="huobao-grid">
          <label class="field">
            <span class="field-label"
              >Huobao API Key
              <span class="dim">(统一用于文本 / 图片 / 视频 / 音频)</span></span
            >
            <input
              v-model="huobaoForm.apiKey"
              class="input"
              type="password"
              placeholder="用于 api.chatfire.site 全链路服务"
            />
          </label>
        </div>
        <div class="preset-grid compact">
          <article
            v-for="preset in huobaoPresetCards"
            :key="`${preset.serviceType}-${preset.provider}`"
            class="preset-card"
          >
            <div class="preset-card-top">
              <span class="preset-service">{{ preset.label }}</span>
        ...
        <div class="modal-actions">
          <button type="button" class="btn" @click="presetDialog = false">
            取消
          </button>
    ...
    >
      <form class="modal card" @submit.prevent="confirmAddSkill">
        ...
        <div class="modal-actions">
          <button type="button" class="btn" @click="addSkillDialog = false">
            取消
          </button>
          <button
            type="submit"
            class="btn btn-primary"
            :disabled="!newSkillForm.id"
          >
            创建
          </button>
        </div>
...
const baseTabs = [{ id: "ai", label: "AI 服务", icon: Cpu }];
const advancedTabs = [
...
const serviceMeta = {
  text: {
    label: "文本",
    desc: "剧本改写、角色场景提取、分镜拆解等 Agent 文本能力",
  },
  image: {
    label: "图片",
    desc: "角色图、场景图、镜头图与首尾帧等静态图像生成",
  },
...
function byType(t) {
  return cfgs.value.filter((c) => c.service_type === t);
}
function countActive(t) {
  return byType(t).filter((c) => c.is_active).length;
}
function fmtModel(m) {
...
function presetsByType(type) {
  ...
  return Object.entries(group).map(([provider, preset]) => ({
    provider,
    ...preset,
  }));
}
  ...
  try {
    for (const preset of huobaoPresetCards) {
      const existing = cfgs.value.find(
        (c) =>
          c.service_type === preset.serviceType &&
...
async function saveAgentCfg(type) {
  ...
  try {
    ...
    toast.success(
      `${agentDefs.find((a) => a.type === type)?.label} 配置已保存`,
    );
    setTimeout(() => {
...
const selectedAgentLabel = computed(
  () => agentDefs.find((a) => a.type === selectedAgent.value)?.label || "",
);
const selectedAgentIcon = computed(
  () => agentDefs.find((a) => a.type === selectedAgent.value)?.icon || "",
);

function agentSkillCount(type) {
  return allSkills.value.filter(
    (s) => s.id === type || s.id.startsWith(type + "/"),
  ).length;
}
...
const currentSkills = computed(() =>
  allSkills.value.filter(
    (s) =>
      s.id === selectedAgent.value ||
      s.id.startsWith(selectedAgent.value + "/"),
  ),
);
